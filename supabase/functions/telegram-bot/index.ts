...
// Telegram chat gets bound to their HabeshaHome account, plus /stop and
...
const COPY = {
  en: {
    ...
    invalid:
      "This link is invalid or has already been used. Open your HabeshaHome profile and tap “Connect Telegram” to get a fresh one.",
    expired:
      "This link has expired — they're valid for 15 minutes. Open your HabeshaHome profile and tap “Connect Telegram” for a new one.",
    noToken:
      `Hello! 👋 I send HabeshaHome alerts.\n\nTo connect, open <a href="${SITE_URL}/profile?connect=telegram">your profile on addisfurnish.vercel.app</a> and tap “Connect Telegram”.`,
    joinAsk:
    ...
    shareNoPending:
      "Start from your HabeshaHome profile — open “Verify phone” there and I'll take it from here.",
    codeSent: (code: string) =>
      `✅ Number confirmed.\n\nYour verification code is <b>${code}</b>\n\nType it on the HabeshaHome page you came from. It expires in 10 minutes.`,
    // ── Language ──
    langAsk: "Choose the language for this bot:\n\nቋንቋ ይምረጡ:",
    langSet: (l: string) => `✅ Language set to <b>${l}</b>.`,
    // ── Shortcut commands ──
    findIntro:
      "🔎 <b>Browse HabeshaHome</b> — sofas, beds, desks and more from shops across Ethiopia.",
  ...
  am: {
    ...
    invalid:
      "ይህ ሊንክ ዋጋ የለውም ወይም አስቀድሞ ተጠቅሟል። የHabeshaHome መገለጫዎን ከፍተው “ቴሌግራም አገናኝ” ይጫኑ።",
    expired:
      "የዚህ ሊንክ ጊዜ አልፎበታል — ለ15 ደቂቃ ብቻ ይሰራል። የHabeshaHome መገለጫዎን ከፍተው አዲስ ያግኙ።",
    noToken:
      `ሰላም! 👋 የHabeshaHome ማሳወቂያዎችን እልካለሁ።\n\nለማገናኘት <a href="${SITE_URL}/profile?connect=telegram">በaddisfurnish.vercel.app ላይ መገለጫዎን</a> ከፍተው “ቴሌግራም አገናኝ” ይጫኑ።`,
    joinAsk:
    ...
    notLinked:
      `ይህ ውይይት ከHabeshaHome መለያ ጋር አልተገናኘም።\n\n<a href="${SITE_URL}/profile?connect=telegram">የHabeshaHome መገለጫዎን</a> ይክፈቱ፣ ይግቡ እና “ቴሌግራም አገናኝ” ይጫኑ።`,
    help: (site: string) =>
      `የHabeshaHome ማሳወቂያዎችን አደርሳለሁ — አዲስ መልእክቶች፣ የጥሪ ጥያቄዎች እና ከምርጫዎ ጋር የሚስማሙ ዕቃዎች።\n\n/start — መለያዎን ያገናኙ\n/join — ቻናላችንን ይቀላቀሉ (ማሳወቂያ ከመጀመሩ በፊት ያስፈልጋል)\n/find — ገበያውን ያስሱ\n/saved — የተቀመጡ እቃዎችዎ\n/mylistings — ንቁ ማስታወቂያዎችዎ\n/inquiries — መልእክቶችዎ\n/alerts — የተቀመጡ ፍለጋዎችን ያስተዳድሩ\n/account — የመለያ ማጠቃለያ\n/sell — ከዚህ የረቂቅ ማስታወቂያ ይፍጠሩ\n/lang — ቋንቋ ይቀይሩ (English / አማርኛ)\n/stop — ማሳወቂያ ያቁሙ\n/help — ይህ መልእክት${
        site ? `\n\n${site}` : ""
      }`,
    ...
    shareNoPending: "ከHabeshaHome መገለጫዎ ይጀምሩ — “ስልክ አረጋግጥ” የሚለውን ይክፈቱ።",
    codeSent: (code: string) =>
      `✅ ቁጥሩ ተረጋግጧል።\n\nየማረጋገጫ ኮድዎ <b>${code}</b> ነው\n\nመጥተውበት ባለው የHabeshaHome ገጽ ላይ ያስገቡት። በ10 ደቂቃ ውስጥ ያልፋል።`,
    // ── Language ──
    langAsk: "ለዚህ ቦት ቋንቋ ይምረጡ:\n\nChoose the language for this bot:",
    langSet: (l: string) => `✅ ቋንቋ ወደ <b>${l}</b> ተቀይሯል።`,
    // ── Shortcut commands ──
    findIntro:
      "🔎 <b>HabeshaHomeን ያስሱ</b> — ከመላ ኢትዮጵያ ሱቆች ሶፋዎችን፣ አልጋዎችን፣ ጠረጴዛዎችን እና ሌሎችንም ያገኛሉ።",
 ...
 * keyboard. Falls back to the HabeshaHome logo when no real photo exists,
...
Deno.serve(async (req) => {
  ...
  if (callback?.data === "verify_channel_join") {
    // Everything below must end in the button being answered and the user
    // seeing SOME message — a silent dead end (spinner, then nothing) is the
  ...
  if (text === "/help") {
    await sendPhoto(chatId, null, copy.help(SITE_URL), {
      inline_keyboard: [[{ text: "🌐 Open HabeshaHome", url: SITE_URL }]],
    });
    return new Response("ok", { status: 200 });
  }
    ...
    if (!token) {
      ...
      if (current) {
        ...
        await sendPhoto(chatId, null, copy.noToken, {
          inline_keyboard: [[{ text: "🌐 Open HabeshaHome", url: `${SITE_URL}/profile?connect=telegram` }]],
        });
      }
      return new Response("ok", { status: 200 });
    }
