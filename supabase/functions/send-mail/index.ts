 ...
 * - The sender address comes from BREVO_SENDER and, until HabeshaHome owns a
...
const SENDER_NAME = Deno.env.get("BREVO_SENDER_NAME") ?? "HabeshaHome";
...
function renderCodeEmail(mailerType: string, token: string): string {
  ...
  return (
    ...
    `<p style="color:#666;font-size:12px;">HabeshaHome — Ethiopia's marketplace for quality second-hand furniture.</p>`
  );
}
...
Deno.serve(async (req) => {
  ...
  const subject = String(
    email.subject ??
      (mailerType === "recovery"
        ? "Reset Your Password"
        : "Verify your email for HabeshaHome"),
  );
