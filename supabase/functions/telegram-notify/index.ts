...
// HabeshaHome logo instead of plain text with a raw link.
...
function copyFor(type: string, payload: NotifPayload, lang: Lang): string {
  ...
  const en: Record<string, string> = {
    ...
    seller_verified: "✅ Your shop has been verified on HabeshaHome. Your badge is now live.",
  ...
  const am: Record<string, string> = {
    ...
    seller_verified: "✅ ሱቅዎ በHabeshaHome ተረጋግጧል። የማረጋገጫ ምልክትዎ አሁን ይታያል።",
  ...
  return (table[type] ?? en[type] ?? `HabeshaHome: ${listing}`) + link;
}
