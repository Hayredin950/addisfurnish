...
export const Route = createFileRoute("/favorites")({
  head: () => ({
    meta: [
      { title: "Saved Furniture — SuqBet" },
      { name: "description", content: "Every listing you saved, in one place." },
      { property: "og:title", content: "Saved Furniture — SuqBet" },
...
function Favorites() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12">
      <h1 className="font-display text-3xl font-semibold">Saved items</h1>
      {data && data.length > 0 ? (
        ...
        <div className="mt-8 rounded-lg border border-dashed p-16 text-center">
          ...
          <Button asChild variant="outline" className="mt-5">
            <Link to="/browse">Find something</Link>
          </Button>
        </div>
      )}
    </div>
